...
def write_dicom_file_data(dicom_file_data, name, cnt):
    directory = f'dicom_files_import/{name}'
    if not os.path.exists(directory):
        os.makedirs(directory)
    with open(os.path.join(directory, f'{cnt}.dcm'), 'wb') as file:
        ...
        for series in series_list:
            ...
            for instance in instances:
                ...
                if dicom_file_data is not None:
                    write_dicom_file_data(dicom_file_data, name, cnt)
                    cnt += 1
    ...
    for study in studies:
        ...
        if study_name == name:
            ...
            for series in series_list:
                ...
                cnt = 0
                for instance in instances:
                    print(instance['MainDicomTags']['InstanceNumber'])
                    break
                    # dicom_file_data = get_dicom_file_data(instance)
                    # if dicom_file_data is not None:
                    #     write_dicom_file_data(dicom_file_data, name, cnt)
